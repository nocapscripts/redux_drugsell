...
if Config.Framework == 'qb' then
    ...
    function Framework:GetCops()
        local amount = 0
        for _, v in pairs(NPX.Functions.GetPlayers()) do
            local Player = NPX.Functions.GetPlayer(v)
            if Player ~= nil then
...
if Config.Inventory == 'ox' then
    ...
    function Framework:GetItem(player, item, meta, returns)
        return exports.ox_inventory:GetItem(player, item, meta, returns)
    end
end
